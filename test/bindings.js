    ...
    describe('instance method', function() {
      ...
      describe('#read', function() {
        ...
        it("doesn't error if the port is open", function(done) {
          binding.read(readyData.length, done);
        });

        it('returns when data becomes available', function(done) {
          var newData = new Buffer('!');
          binding.read(readyData.length, function(err) {
            assert.isNull(err);
            binding.read(5, function(err, data) {
              assert.isNull(err);
        ...
        it('returns at maximum the requested number of bytes', function(done) {
          binding.read(4, function(err, data) {
            assert.deepEqual(data, readyData.slice(0, 4));
            done();
          });
        });
      });
